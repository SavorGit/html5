...
$(document).ready(function(){
	...
	$('.row > .right').click(function(e){
		...
		var postData = {
			is_js: 1,
			openid: $(this).parent('.row').attr('open-id'),
			goods_id: $(this).parent('.row').attr('goods-id')
		};
		console.log('Ajax[var]', Page.Exchange.api.url, postData);
		JohnLeeConsole.print('Ajax[var]', Page.Exchange.api.url, postData);
		$.ajax({
			url: Page.Exchange.api.url,
			type: "POST",
			cache: false,
			data: postData,
			dataType: "JSONP",
