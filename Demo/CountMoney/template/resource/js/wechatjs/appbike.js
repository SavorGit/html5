define('appbike',['appsocket','jquery.shake','wechatcore','touch'],function(appsocket,Shake,wechatcore,touch){
	...
	var appbike = {
		...
		init: function() {
			...
			params.onmessage = function(data){
				...
				}else if(data.type=='gamebikerefresh'||data.type=='gameappbikerefresh'){
		...
		hidephbbtn:function(){
			$(".zam-app-shake-over .phb-btn").addClass('hidden');
		},
		changeCirclImg:function(type){
			...
			$(".zam-app-shakehand").css({"background-image":"url("+img+")"});
		}
	};
	return appbike;
});
