define('screenjjb',['screensocket','screencore','template','layerBox','fulldm','keyboard'],function(screensocket,screencore,template,layerBox,fulldm,keyboardJS){
	var screenjjb = {
		gamestatus:0,
		stopclick:false,
		maxscore: 120,
		...
		init: function() {
			...
			params.onmessage = function(data){
				...
				}else if(data.type=='gamejjbrefresh'||data.type=='gameerrorhaddone'){
		...
		rushDom:function(list){
			...
			f.each(function(n) {
				...
				var rh = 50;
				if (l > 0) {//max 540
					var percent = l / self.maxscore;
					rh += parseInt(490 * percent);
				} 
				if(l > parseInt(q.find(".shu-rank-item-score").text())){
					q.find('.drop').addClass('drop1');
					q.find('.drop').one('webkitAnimationEnd mozAnimationEnd MSAnimationEnd oanimationend animationend', function(){
				...
				q.find(".shu-rank-item-score").text(l);
				q.find(".shu-rank-item-info > img").attr("src", o)
			});
		},
