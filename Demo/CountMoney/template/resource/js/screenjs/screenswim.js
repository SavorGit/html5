define('screenswim',['screensocket','screencore','template','layerBox','fulldm','keyboard'],function(screensocket,screencore,template,layerBox,fulldm,keyboardJS){
		var screenswim = {
		gamestatus:0,
		stopclick:false,
		maxscore: 120,
		...
		init: function() {
			...
			params.onmessage = function(data){
				if(data.type=='connected'){
					...
					data.djs==0&&$(".swim-box").addClass('runbg2');
				...
				}else if(data.type=='gameswimrefresh'||data.type=='gameerrorhaddone'){
		...
		rushDom:function(list){
			...
			l.each(function(i,v){
				if(list[i]){
					var bottom = (list[i].score/self.maxscore).toFixed(2);
					if(bottom>=1){
						bottom = "calc(100% - 200px)";
					}else{
						bottom = "calc("+(bottom*100)+"% - 200px)";
					}
		...
		createLine:function(){
			...
			$(".shakelineBox li").each(function(key,ele) {
				var onePlayer = $(this);
				onePlayer.css({
					"width":($(".shakelineBox").width()/10)+'px',
					"height":($(".shakelineBox").width()/10)*4/3,
					"left":($(".shakelineBox").width()/10*key)+'px',
				});
				onePlayer.find('span').css({"top":(onePlayer.height()+45)+'px'});
			});
		},
		resize:function(){
			$(".shakelineBox li").each(function(key,ele) {
