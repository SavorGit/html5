...
function qp_w() {
	...
	a.on("pressup",
	function(a) {
		IS_TOUCH && a.nativeEvent instanceof MouseEvent || 2 != qp_o || (50 < c - a.localY ? (a = (new Date).getTime(), 0 < qp_i.length && qp_i[qp_i.length - 1] + 50 > a ? qp_a("WARNING: Too fast! maybe engine error.") : (f = qp_y(a), f <= qp_j ? (qp_k++, MyScore += 100, qipaStage.stage.player.playAnimation(qipaStage.stage.player.m[qp_f]), createjs.Sound.play("count", !0)) : (qp_i.length--, qp_a("WARN: " + f)))) : (qp_z(d), qipaStage.stage.player.m[qp_f].visible = !1))
	});
	qp_c = [];
	...
	function(a) {
		...
		qipaStage.stage.num.sum.text = "\uffe5" + MyScore
	})
}
function Qp_A() {
	this.initialize();
...
Qp_C.prototype.refresh = function() {
	this.scoreText.text = "\uffe5" + MyScore;
	//this.shareText.text = 0 < MyScore ? dataForWeixin.tContent.replace("\u6bd4", "\n\u6bd4").replace("\u6211\u662f", "\n\u6211\u662f") : ""
};
function Qp_B() {
	this.initialize();
	...
	this.sum = new createjs.Text("\uffe5" + MyScore, "bold 46px Arial", "yellow");
...
function qp_u() {
	// dataForWeixin.tTitle = "\u6570\u94b1\u6570\u5230\u624b\u62bd\u7b4b\uff01\u4f60\u662f\u6570\u94b1\u9ad8\u624b\u5417\uff1f";
	// if (0 == MyScore) dataForWeixin.tContent = dataForWeixin.tTitle;
	// else {
	// 	var a = parseInt(Math.sqrt(1E4 * MyScore / 17E3));
	// 	99 < a && (a = "99.9");
	// 	dataForWeixin.tTitle = "\u6211\u6570\u4e86\uffe5" + MyScore + "\uff0c\u6bd4" + a + "%\u7684\u4eba\u6709\u94b1\uff01\u6211\u662f" + (5E3 > MyScore ? "\u5c4c\u4e1d": 1E4 > MyScore ? "\u8d2b\u519c": 15E3 > MyScore ? "\u5bcc\u519c": 2E4 > MyScore ? "\u571f\u8c6a": 25E3 > MyScore ? "\u7164\u8001\u677f": "\u8d44\u672c\u5bb6") + "\u3002"
	// }
}
var _cfg = {
	startFunc: qp_v,
	img: {
		path: "./template/resource/images/mobile/shuqian/",
		manifest: [{
			src: "m0.png",
			id: "m0"
		},
	...
	audio: {
		path: "./template/resource/images/mobile/",
		manifest: [{
			src: "count.mp3",
			id: "count"
		}]
