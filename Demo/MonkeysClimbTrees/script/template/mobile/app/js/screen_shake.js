...
function showGameResult() {
    ...
    var c = game_config.scene_info.win_count;
    $(".button.allresult", a).show();
    $(".button.nexttound").show();
    //document.getElementById("Audio_Gameover").play();
    Audio.play("#Audio_Gameover");
    window.setTimeout(function() {
        d.fadeOut(function() {
            a.show(function() {
                if (c >= 1 && game_config.scene_info.rankTopTen[0]) {
                    window.setTimeout(function() {
                        ...
                        e.find(".head").css({
                            "background-image": "url(" + game_config.scene_info.rankTopTen[0]["userHeader"] + ")"
                        }).addClass("shake");
                        e.find(".nickname").html(game_config.scene_info.rankTopTen[0]["userNickname"]);
                        e.appendTo(a).addClass("bounce");
                    }, 800)
                }
                if (c >= 2 && game_config.scene_info.rankTopTen[1]) {
                    window.setTimeout(function() {
                        var e = $PlayeSeed.clone().addClass("result").css({
                            left: "40px",
                            width: "100px",
                        ...
                        e.find(".head").css({
                            "background-image": "url(" + game_config.scene_info.rankTopTen[1]["userHeader"] + ")"
                        }).addClass("shake");
                        e.find(".nickname").html(game_config.scene_info.rankTopTen[1]["userNickname"]);
                        e.appendTo(a).addClass("bounce");
                    }, 1800)
                }
                if (c >= 3 && game_config.scene_info.rankTopTen[2]) {
                    window.setTimeout(function() {
                        var e = $PlayeSeed.clone().addClass("result").css({
                            right: "30px",
                            width: "70px",
                        ...
                        e.find(".head").css({
                            "background-image": "url(" + game_config.scene_info.rankTopTen[2]["userHeader"] + ")"
                        }).addClass("shake");
                        e.find(".nickname").html(game_config.scene_info.rankTopTen[2]["userNickname"]);
                        e.appendTo(a).addClass("bounce");
                    }, 2800)
                }
            })
...
var cutdown_start = function() {
    var a = $(".cutdown-start");
    var b = (game_config.timers.ready.duration / game_config.timers.ready.interval) * 1 + 1;
    a.html("").show().css({
    ...
    tmr_cutdown_start = window.setInterval(function() {
        ...
        if (b == 0) {
            ...
            a.hide();
            */
            if (game_config.user && game_config.user.speed){
			...
			*/
            ...
            Audio.play("#Audio_CutdownPlayer");
            a.html(b);
            if (game_config.user && game_config.user.speed){
	        	game_config.user.speed = 0;
	        }
